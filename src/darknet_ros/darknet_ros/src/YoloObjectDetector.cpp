    ...
    for (j = 0; j < demoClasses_; ++j) {
      if (dets[i].prob[j]) {
        ...
        if (BoundingBox_width > 0.01 && BoundingBox_height > 0.01) {
          ...
          printf("\nx_center:%.6f\n",roiBoxes_[count].x);//add by liugang
          printf("\ny_center:%.6f\n",roiBoxes_[count].y);//add by liugang
          printf("\nBoundingBox_width:%.6f\n",roiBoxes_[count].w);//add by liugang
          printf("\nBoundingBox_height:%.6f\n",roiBoxes_[count].h);//add by liugang
          printf("\nClass:%d\n",roiBoxes_[count].Class);//add by liugang
          count++;
